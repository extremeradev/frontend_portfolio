<template>
  <div v-for="skill in skills" :key="skill.id" class="card-container w-full flex items-center justify-center ">
...
.card-container {
  perspective: 1000px;
  /* Define la distancia de la cámara para el efecto 3D */
}
...
.card {
  ...
  /* Mantener la estructura 3D para las caras */
...
.card-back {
  ...
  /* Esconde el reverso cuando se gira */
  display: flex;
  align-items: center;
...
.card-back {
  ...
  /* Gira el reverso 180 grados para que no sea visible */
}

/* Efecto al hacer hover (cuando el ratón pasa por encima de la tarjeta) */
.card-container:hover .card {
  transform: rotateY(180deg);
  /* Gira la tarjeta 180 grados */
}
</style>